		...
		function get_stories_html($req_vars) {
			...
			if((int)($st_limit) == 0) {
				if(isset($req_vars['search_option']) && ($req_vars['search_option'] == 'search')) {
					$story_cursor = $this->db->stories->find(array('story_title' => $req_vars['search_string']))->sort(array($req_vars['order_by'] => (int)$req_vars['order']))->limit((int)$req_vars['limit']);
				} else {
					$story_cursor = $this->db->stories->find()->sort(array($req_vars['order_by'] => (int)$req_vars['order']))->limit((int)$req_vars['limit']);
				}
			} else {
				if(isset($req_vars['search_option']) && ($req_vars['search_option'] == 'search')) {
					$story_cursor = $this->db->stories->find(array('story_title' => $req_vars['search_string']))->sort(array($req_vars['order_by'] => (int)$req_vars['order']))->limit($req_vars['limit'])->skip((int)($st_limit));
				} else {
